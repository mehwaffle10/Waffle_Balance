...
class StructurePNGLoader
{
	...
	void handlePixel(const SColor &in pixel, int offset)
	{
		...
		else
		{
			...
			case map_colors::tile_thickstone:       map.server_SetTile(map.getTileWorldPosition(offset), CMap::tile_thickstone);       break;
			case map_colors::tile_bedrock:          map.server_SetTile(map.getTileWorldPosition(offset), CMap::tile_bedrock);          break;
			case map_colors::tile_gold:             map.server_SetTile(map.getTileWorldPosition(offset), CMap::tile_gold);             break;
			case map_colors::tile_castle:           map.server_SetTile(map.getTileWorldPosition(offset), CMap::tile_castle);           break;
			case map_colors::tile_castle_back:      map.server_SetTile(map.getTileWorldPosition(offset), CMap::tile_castle_back);      break;
			case map_colors::tile_castle_moss:      map.server_SetTile(map.getTileWorldPosition(offset), CMap::tile_castle_moss);      break;
			case map_colors::tile_castle_back_moss: map.server_SetTile(map.getTileWorldPosition(offset), CMap::tile_castle_back_moss); break;
			case map_colors::tile_wood:             map.server_SetTile(map.getTileWorldPosition(offset), CMap::tile_wood);             break;
			case map_colors::tile_wood_back:        map.server_SetTile(map.getTileWorldPosition(offset), CMap::tile_wood_back);        break;
			case map_colors::tile_grass:            map.server_SetTile(map.getTileWorldPosition(offset), CMap::tile_grass + map_random.NextRanged(3)); break;
			...
			case map_colors::spikes_ground: offsets[spike_offset].push_back(offset); map.server_SetTile(map.getTileWorldPosition(offset), CMap::tile_ground_back); break;
			case map_colors::spikes_castle: offsets[spike_offset].push_back(offset); map.server_SetTile(map.getTileWorldPosition(offset), CMap::tile_castle_back); break;
			case map_colors::spikes_wood:   offsets[spike_offset].push_back(offset); map.server_SetTile(map.getTileWorldPosition(offset), CMap::tile_wood_back);   break;
	...
	void handleOffset(int type, int offset, int position, int count)
	{
		...
		case tree_offset:
		{
			...
			if(tree !is null)
			{
				...
				{
					map.server_SetTile(map.getTileWorldPosition(offset), CMap::tile_grass + map_random.NextRanged(3) );
				}
			}
		}
		break;
...
void PlaceMostLikelyTile(CMap@ map, int offset)
{
	...
	if (up != CMap::tile_empty)
	{
		...
		{
			map.server_SetTile(map.getTileWorldPosition(offset), CMap::tile_castle_back);
		}
		else if ((neighborhood.find(CMap::tile_wood) != -1) ||
		         (neighborhood.find(CMap::tile_wood_back) != -1))
		{
			map.server_SetTile(map.getTileWorldPosition(offset), CMap::tile_wood_back );
		}
	...
	{
		map.server_SetTile(map.getTileWorldPosition(offset), CMap::tile_grass + 2 + map_random.NextRanged(2));
	}
}
...
{
    // Waffle: Base team off of position
    string team = map.getTileSpacePosition(offset).x < map.tilemapwidth / 2 ? "blue" : "red";
	map.AddMarker(map.getTileWorldPosition(offset), name.replace("red", team).replace("blue", team));
}
