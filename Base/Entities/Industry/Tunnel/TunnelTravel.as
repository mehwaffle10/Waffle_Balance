...
void Travel(CBlob@ this, CBlob@ caller, CBlob@ tunnel)
{
	...
	if (thisTunnel !is null && caller !is null && tunnel !is null)
	{
		...
		// Waffle: Deny tunnel travel if carrying a flag
		if (isHoldingFlag(caller))
		{
			return;
		}

		// Waffle: Prevent pickup teleport bug
		caller.set_u32("TUNNEL_TRAVEL_PICKUP_DELAY", getGameTime());
...
// Waffle: Check if holding flag
bool isHoldingFlag(CBlob@ caller)
{
	CBlob@ attached = caller.getCarriedBlob();
	return attached !is null && attached.getName() == "ctf_flag";
}