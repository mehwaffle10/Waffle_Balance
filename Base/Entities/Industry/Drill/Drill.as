...
void onTick(CBlob@ this)
{
	...
	if (this.isAttached())
	{
		...
		if (holder.getName() == required_class || sv_gamemode == "TDM")
		{
			...
			u8 delay_amount = 8;
			// Waffle: Greatly increase the delay for digging just dirt
			if (this.get_bool("just hit dirt")) delay_amount = 30;
			// Waffle: Double delay instead of a fixed amount in water
			if (inwater) delay_amount *= 2;
			
			bool skip = (gametime < this.get_u32(last_drill_prop) + delay_amount);

			if (skip)
			{
				// Waffle: Heat up quickly if only hitting dirt
				if (this.get_bool("just hit dirt"))
				{
					heat += 1;
					this.set_u8(heat_prop, heat);
					this.Sync(heat_prop, true);
