...
void onTick(CSprite@ this)
{
	...
	if (!blob.hasTag(burning_tag)) //give way to burning anim
	{
		...
		else if (action2 || ((this.isAnimation("slow_strike") || this.isAnimation("strike") || this.isAnimation("fast_strike") || this.isAnimation("chop") || this.isAnimation("fast_chop")) && !this.isAnimationEnded()))
		{
			...
			if (blob.get("hitdata", @hitdata))
			{
				...
				// Waffle: Make builders dig dirt slower
				if (hitting_dirt)
				{
					attack_anim = "slow_strike";
				}
				else if (hitting_wood)
				{
					// Waffle: Make builders dig man made wood faster
					attack_anim = hitting_built ? "fast_chop" : "chop";
				}
				else if(hitting_stone)
				{
					// Waffle: Make builders dig man made stone faster
					attack_anim = hitting_built ? "fast_strike" : "strike";
				}
