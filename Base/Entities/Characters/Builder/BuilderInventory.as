...
void onRender(CSprite@ this)
{
	...
	{
		// no build zone show
		const bool onground = fakeOnGround(map, blob);  // blob.isOnGround(); // Waffle: Make sure there's a supporting block or blob underneath
		...
		if (time + SHOW_NO_BUILD_TIME > getGameTime())
		{
			...
			for (f32 step_x = 0.0f; step_x < space.x ; ++step_x)
			{
				for (f32 step_y = 0.0f; step_y < space.y ; ++step_y)
				{
                    ...
                    // Waffle: Buildings can be placed behind trees or over scenary
                    CMap::Sector@[] sectors;
					...
					if (!onground || no_build || map.isTileSolid(v) || blobBlockingBuilding(map, v))  // || map.getSectorAtPosition(v , "no build") !is null
					{
						// draw red
