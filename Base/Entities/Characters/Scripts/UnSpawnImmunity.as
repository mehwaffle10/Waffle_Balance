...
void onTick(CBlob@ this)
{
	...
	// Waffle: Cancel immunity if using mouse
	if (immunity && this.getTickSinceCreated() > 2 && (this.isKeyPressed(key_action1) || this.isKeyPressed(key_action2))) {
		immunity = false;
	}

	// Waffle: Cancel immunity if you walk off tent
	CBlob@[] blobsInRadius;
	if (immunity && this.getMap().getBlobsInRadius(this.getPosition(), this.getRadius() * 1.1f, @blobsInRadius))
	{
		bool onTent = false;
		for (u16 i = 0; i < blobsInRadius.length; i++)
		{
			CBlob@ blob = blobsInRadius[i];
			if (blob !is null && blob.getName() == "tent" && blob.getTeamNum() == this.getTeamNum())
			{
				onTent = true;
				break;
			}
