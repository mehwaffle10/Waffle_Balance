...
void onRender(CSprite@ this)
{
	...
	{
		// string lastPointName;  // Waffle: Always draw all attachment points
		for (uint i = 0; i < aps.length; i++)
		{
			AttachmentPoint@ ap = aps[i];

			if (ap.getOccupied() is null && ap.socket && ap.getKeysToTake() > 0 && ap.radius > 0.0f) // gather empty controllers attachments/seats  // Waffle: Always draw all attachment points: && lastPointName != ap.name)
			{
				...
				if (occupied == 0 && (!driver || (driver && !blob.isAttached())))
				{
					...
					// Waffle: Always draw all attachment points
					// if (g_debug == 0)
					// {
