...
void onTick(CBlob@ this)
{
	...
	else
	{
		...
		if (timer <= 0)
		{
			if (getNet().isServer())
			{
				// Waffle: Fix faulty screen shake
...
void onCommand(CBlob@ this, u8 cmd, CBitStream @params)
{
	if (cmd == this.getCommandID("activate client") && isClient())
	{
        ...
        this.getSprite().RewindEmitSound();  // Waffle: Fix extinguished kegs not resetting the fuse sound
	}
...
{
	// this.server_SetHealth(-1.0f);  // Waffle: Fix faulty screen shake
	// this.server_Die();
...
{
	this.getSprite().SetEmitSoundPaused(true);
	if (this.hasTag("exploding"))  // Waffle: Fix faulty screen shake
	{
		Boom(this);
	}
}
...
void onCollision(CBlob@ this, CBlob@ blob, bool solid)
{
	...
	if (vellen > 1.7f)
	{
		...
		if (isServer() && this.hasTag("exploding") && vellen > 8.0f)
		{
			// Waffle: Fix faulty screen shake
			this.server_SetHealth(-1.0f);
			this.server_Die();
