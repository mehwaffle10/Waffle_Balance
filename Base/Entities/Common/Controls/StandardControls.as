...
void onCommand(CBlob@ this, u8 cmd, CBitStream @params)
{
	...
	else if (cmd == this.getCommandID("pickup"))
	{
        ...
        if (caller.get_u32("TUNNEL_TRAVEL_PICKUP_DELAY") + 1 * getTicksASecond() >= getGameTime()) return;
        ...
        CMap@ map = getMap();
        if (map is null || (caller.getPosition() - pickedup.getPosition()).Length() > caller.getRadius() + 60.0f)
        {
            return;
        }
