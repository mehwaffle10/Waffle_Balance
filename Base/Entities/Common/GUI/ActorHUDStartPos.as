...
void DrawResupplyOnHUD(CBlob@ this, Vec2f tl)
{
	...
	string propname = RESUPPLY_TIME_STRING;  // getCTFTimerPropertyName(p, "builder");  // Waffle: Resupply crate not builder resupply

	if (!getRules().exists(propname)) return;
	int wood_amount = crate_wood_amount;    // matchtime_wood_amount;   // Waffle: Resupply crate not builder resupply
    ...
    // Waffle: Update text
	string resupply_unavailable = "Next airdrop of {WOOD} wood and {STONE} stone in {SEC}{TIMESUFFIX}." // getTranslatedString("Next resupply of {WOOD} wood and {STONE} stone in {SEC}{TIMESUFFIX}.")
		.replace("{SEC}", "" + secs)
	...
	if (!isBuildPhase(getRules()))  // if (next_items > getGameTime())  // Waffle: Crate during buildphase
	{
