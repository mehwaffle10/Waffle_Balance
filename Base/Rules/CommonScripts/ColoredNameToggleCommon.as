...
// Waffle: Change purple color from devs to superadmins
bool isSuperAdmin(CPlayer@ player)
{
    return getSecurity().checkAccess_Feature(player, "superadmin_color");
}

bool isAdmin(CPlayer@ player)
{
    return isSuperAdmin(player) || getSecurity().checkAccess_Feature(player, "admin_color") || player.isRCON();
}

bool isSpecial(CPlayer@ player)
{
    return player.isGuard() || isAdmin(player);  // player.isDev() // Waffle: Change purple color from devs to superadmins
}
